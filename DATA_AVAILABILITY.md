...
**Requesting access.** Contact ABHL directly. The authors don't own this data and can't grant access to it on ABHL's behalf; this repository doesn't gatekeep access to data it doesn't control. Current contact information is in the Zenodo record for this repository: ==FILL_ZENODO_DOI_HERE==.

## FAIR

Everything in the open-data table is Findable and Accessible through this repository and its Zenodo archive (==FILL_ZENODO_DOI_HERE==), Interoperable as CSV or JSON with the schemas the code itself defines, and Reusable under CC-BY 4.0 (weather data) or MIT (code and derived output). The restricted dataset is not FAIR by design. It sits under third-party access control, and this repository doesn't work around that.
